...
%---------------------------Varying Lead time------------------------------
result_operational_emission2=[];
result_carbon_emission2 = [];

m=1;
M=350;                                        %Constant demand
QRcbpt = M;                                   %Initializing quantity in regular time
QOcbpt = 0;                                   %Initializing quantity in over time
Dbi(1) = M*1*b(1);                            %Calculation of demand by plants from supplier using constraint 17
Dbi(2) = M*1*b(2)+M*1*b(3);
...
for LT=1:7
  ...
  y2 = d1_2+d2_2+d3_2+d4_2;
  result_operational_emission2(m) = min(y1);
  result_carbon_emission2(m) = min(y2);
  m=m+1;
...
figure
hold on
[haX,Line1,Line2] = plotyy(LT,result_operational_emission2,LT,result_carbon_emission2);
...
fprintf('\n\nTABLE 4\nFinding total tax vs cap and trade\n');
fprintf('Total Tax\t\t\t\tCap and Trade\n');
for i=1:6
   ...
   fprintf('%.2f\t\t\t%.2f',total_cost(i),cap_trade(i));
   fprintf('\n');
end
...
%For finding graph for strict capping vs Cap and trade (for varying carbon cap)
fine_cap = [0,0,0,0,0,0,0];
Ccap = [30000,32500,35000,37500,40000,42500,45000]; 
fprintf('\n\nTABLE 5\nFinding Strict capping vs Cap and Trade\n');
fprintf('Total Tax\t\t\t\tCap and Trade\n');
for i=1:7 
    if(result_carbon_emission(i)>Ccap(i))
      fine_cap(i) = result_operational_emission(i);
    else 
      fine_cap(i) = result_operational_emission(i); 
    end
    if(result_carbon_emission(i)>Ccap(i))
   ...
   fprintf('%.2f\t\t\t%.2f',fine_cap(i),cap_trade(i));
   fprintf('\n');
end
